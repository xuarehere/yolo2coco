...
LastEditTime: 2023-01-09 16:10:06
LastEditors: xuarehere
Description: 类别合并、映射、丢弃
FilePath: /special_site_2d_220801/merge_labels.py
...
def yaml_load(ymal_file):
    with open(ymal_file, errors='ignore') as f:
        params = yaml.safe_load(f)  # load hyps dict 
    return params  
...
class DataOpts:
    def __init__(self, config="configs/merge_labels.yaml") -> None:
        ...
        # path = ["./VOC2007/JPEGImages",
        #         ]   # 图片地址
        path = self.config["BASE"]["path"]
        
        # self.save_dir = "./tmp/"    # 被丢弃的信息，储存地址。如：图片中的所有的label都不在合并的类型中，因此丢弃
        self.save_dir = self.config["BASE"]["save_dir"]    # 被丢弃的信息，储存地址。如：图片中的所有的label都不在合并的类型中，因此丢弃
    ...
    def init_param(self):
        ...
        #                     'TourBus', 
        #                     'Truck']# 原来的类别
        self.old_labels = self.config["OLD"]["old_labels"]
        ...
        self.new_labels =  self.config["NEW"]["new_labels"]
        ...
        #     "CoachesCar": [],
        # }    # 合并的   
        self.merge_labels = self.config["NEW"]["merge_labels"]
        pass
...
def parse_opt(known=False):
    ...
    parser.add_argument('--config', type=str, default='./configs/merge_labels.yaml', help='configs/merge_labels.yaml path')
    return parser.parse_known_args()[0] if known else parser.parse_args()
        
def main():
    opt = parse_opt()
    params = opt
    config= yaml_load(params.config)    
    run =  DataOpts(config)
    run.piplie()
    pass
